...
# Taking a sample of the last 15 years 
df_salary <- df_salary %>% filter( season_start >= 2003 )
...
# Checking 
df_dropped <- df_salary[df_salary$player_id %in% names(which(table(df_salary$player_id) <= 5)), ]
...
write.csv(df_full, "/Users/steve_j/Documents/CEU /data_analysis/DA_2/DA2_Term//nba_clean13.csv", row.names = FALSE)
